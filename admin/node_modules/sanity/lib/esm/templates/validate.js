...
export function validateTemplates(schema, templates) {
  ...
  templates.forEach((template, i) => {
    ...
    if (idMap.has(template.id)) {
      ...
      const dupe = "".concat(quote(templates[dupeIndex].title), " at index ").concat(dupeIndex);
...
export function validateInitialObjectValue(value, template) {
  const contextError = msg => "Template \"".concat(template.id, "\" initial value: ").concat(msg);
...
function validateValue(value) {
  ...
  let parentIsArray = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

  if (Array.isArray(value)) {
    return value.map((item, i) => {
      if (Array.isArray(item)) {
        throw new Error("multidimensional arrays are not supported (at path \"".concat(pathToString(path), "\")"));
      }
  ...
  const initial = parentIsArray && !value._key ? {
    _key: randomKey()
...
function validateReference(value) {
  ...
  if (!value._type && value.type) {
    throw new Error("Reference is missing \"_type\", but has a \"type\" property at path \"".concat(pathToString(path), "\""));
  }
  ...
  if (disallowed.length > 0) {
    ...
    throw new Error("Disallowed ".concat(plural, " found in reference: ").concat(disallowed.map(quote).join(', '), " at path \"").concat(pathToString(path), "\""));
  }
}