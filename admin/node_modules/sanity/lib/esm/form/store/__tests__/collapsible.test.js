...
test("doesn't make primitive fields collapsed even if they are configured to be", () => {
  ...
  const result = prepareFormState({ ...DEFAULT_PROPS,
    schemaType: bookType,
    document: {
      _id: 'foo',
      _type: 'book'
...
function getDeepFieldMember(objectFormNode, path) {
  ...
  if (!nextField) {
    throw new Error("Field with name \"".concat(head, "\" not found for object node at path \"").concat(pathToString(objectFormNode.path), "\""));
  }
