...
export function useFocusBlock() {
  ...
  const selection = usePortableTextEditorSelection(); // eslint-disable-next-line react-hooks/exhaustive-deps
...
export function useActionGroups(_ref) {
  let {
    hotkeys,
    onExpand,
    resolveInitialValue,
...
export function useActiveActionKeys(_ref2) {
  let {
    actions
  } = _ref2;
  const editor = usePortableTextEditor();
...
export function useActiveStyleKeys(_ref3) {
  let {
    items
  } = _ref3;
  const editor = usePortableTextEditor();
  ...
  const selection = usePortableTextEditorSelection();
  return useUnique(useMemo(() => items.filter(i => PortableTextEditor.hasBlockStyle(editor, i.style)).map(i => i.style), //  eslint-disable-next-line react-hooks/exhaustive-deps
