...
export function extractPastedFiles(dataTransfer) {
  ...
  return normalizeItems(Array.from(dataTransfer.items || [])).then(_flatten);
}
export function extractDroppedFiles(dataTransfer) {
  ...
  return normalizeItems(items).then(_flatten);
}
...
function walk(entry) {
  ...
  if (entry.isDirectory) {
    ...
    return new Promise((resolve, reject) => dir.readEntries(resolve, reject)).then(entries => entries.filter(entr => !entr.name.startsWith('.'))).then(entries => Promise.all(entries.map(walk)).then(_flatten));
  }
