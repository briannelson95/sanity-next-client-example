...
export function fileTarget(Component) {
  return /*#__PURE__*/React.forwardRef(function FileTarget(props, ref) {
    const {
      onFiles,
      onFilesOver,
      onFilesOut,
    ...
    const pasteInput = React.useRef(null);
    const forwardedRef = useForwardedRef(ref);
    const enteredElements = React.useRef([]);
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Component, _extends({}, rest, {
      ref: forwardedRef,
    ...
    })), !disabled && showPasteInput && /*#__PURE__*/React.createElement("div", {
...
function convertImagesToFilesAndClearContentEditable(element) {
  ...
  }).then(images => Promise.all(images.map(img => imageUrlToBlob(img.src)))).then(imageBlobs => imageBlobs.map(blob => new File([blob], 'pasted-image.jpg', {
    type: targetFormat
  })));
} // needed by Edge
