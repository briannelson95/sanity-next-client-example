...
import { useEffect, useState } from 'react'; // http://probablyprogramming.com/2009/03/15/the-tiniest-gif-ever
...
export function useLoadImage(url) {
  const [state, setState] = useState(INITIAL_STATE);
  useEffect(() => {
    setState(INITIAL_STATE);
    const subscription = loadImage(url) // .pipe(delay(2000))
    .subscribe({
      error: err => {
        setState({
          isLoading: false,
