...
export function createGetReferenceInfo(context) {
  ...
  referenceType) {
    return (doc._type ? of(doc) : documentPreviewStore.observeDocumentTypeFromId(doc._id, apiConfig).pipe(map(docType => ({
      _id: doc._id,
      _type: docType
    })))).pipe(switchMap(resolvedDoc => {
      if (!resolvedDoc._type) {
        ...
        return fetchDocumentAvailability(client, doc._id).pipe(map(availability => ({
          id: doc._id,
          type: undefined,
          availability,
          preview: {
      ...
      return combineLatest([publishedPreview$]).pipe(map(_ref => {
...
function fetchDocumentAvailability(client, id) {
  ...
  return client.observable.request(requestOptions).pipe(map(response => {
    const omitted = _keyBy(response.omitted || [], entry => entry.id);
