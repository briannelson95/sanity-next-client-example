...
import { getPreviewPaths, prepareForPreview } from '../../../../preview';
import { createWeightedSearch } from '../../../../search';
import { getIdPair, getDraftId, collate, isRecord } from '../../../../util';
const READABLE = {
  available: true,
  reason: 'READABLE'
};
...
export function getReferenceInfo(documentPreviewStore, id, referenceType) {
  const {
    publishedId,
    draftId
  } = getIdPair(id);
  ...
  return pairAvailability$.pipe(switchMap(pairAvailability => {
    ...
    const typeName$ = combineLatest([documentPreviewStore.observeDocumentTypeFromId(draftId), documentPreviewStore.observeDocumentTypeFromId(publishedId)]).pipe( // assume draft + published are always same type
    map(_ref => {
    ...
    return typeName$.pipe(switchMap(typeName => {
      ...
      const previewPaths = [...(getPreviewPaths(refSchemaType === null || refSchemaType === void 0 ? void 0 : refSchemaType.preview) || []), ['_updatedAt'], ['_createdAt']];
      const draftPreview$ = documentPreviewStore.observePaths(draftRef, previewPaths).pipe(map(result => result ? {
        _id: draftId,
        ...prepareForPreview(result, refSchemaType)
      } : undefined), startWith(undefined));
      const publishedPreview$ = documentPreviewStore.observePaths(publishedRef, previewPaths).pipe(map(result => result ? {
        _id: publishedId,
        ...prepareForPreview(result, refSchemaType)
      } : undefined), startWith(undefined));
      const value$ = combineLatest([draftPreview$, publishedPreview$]).pipe(map(_ref2 => {
...
function getExistingCounterparts(client, ids) {
  return ids.length === 0 ? of([]) : client.observable.fetch("*[_id in $ids]._id", {
    ids
  }, {
...
export function referenceSearch(client, textTerm, type, options) {
  const searchWeighted = createWeightedSearch(type.to, client, options);
  return searchWeighted(textTerm, {
    includeDrafts: true
  }).pipe(map(results => results.map(result => result.hit)), map(collate), // pick the 100 best matches
  map(collated => collated.slice(0, 100)), mergeMap(collated => {
