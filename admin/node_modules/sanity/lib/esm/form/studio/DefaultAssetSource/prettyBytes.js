 ...
 */
export function prettyBytes(input) {
  let {
    bits = false,
    binary = false,
    signed = false,
  ...
  const isNegative = number < 0; // eslint-disable-next-line no-nested-ternary
  ...
  if (number < 1) {
    ...
    return "".concat(prefix + numberString, " ").concat(UNITS[0]);
  }
  ...
  return "".concat(prefix + numberString, " ").concat(unit);
}