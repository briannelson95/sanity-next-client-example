 ...
 * @see PaneNode
 */
export async function resolveIntent(options) {
  ...
  const fallbackEditorPanes = [[{
    ...
    params: { ..._omit(options.params, ['id']),
  ...
  async function traverse(_ref) {
    ...
    const resolvedPane = await resolvePane(unresolvedPane, context, flatIndex).pipe(first()).toPromise(); // if the resolved pane is a document pane and the pane's ID matches then
