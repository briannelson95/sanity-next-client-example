...
export function _calcPaneResize(cache, left, right, deltaX) {
  ...
  const leftMinWidth = (_left$minWidth = left.minWidth) !== null && _left$minWidth !== void 0 ? _left$minWidth : PANE_DEFAULT_MIN_WIDTH;
  const rightMinWidth = (_right$minWidth = right.minWidth) !== null && _right$minWidth !== void 0 ? _right$minWidth : PANE_DEFAULT_MIN_WIDTH;
  const leftMaxWidth = Math.min(left.maxWidth || Infinity, sum.width - rightMinWidth);
  const rightMaxWidth = Math.min(right.maxWidth || Infinity, sum.width - leftMinWidth); // Get min. delta X
