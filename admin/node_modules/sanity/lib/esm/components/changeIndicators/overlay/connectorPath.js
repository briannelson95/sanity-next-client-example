import { ARROW_MARGIN_X, ARROW_MARGIN_Y, ARROW_SIZE, ARROW_THRESHOLD, CORNER_RADIUS } from '../constants';
export function arrowPath(x, y, dir) {
  return ["M ".concat(x - ARROW_SIZE, " ").concat(y - ARROW_SIZE * dir, " "), "L ".concat(x, " ").concat(y), "L ".concat(x + ARROW_SIZE, " ").concat(y - ARROW_SIZE * dir)].join('');
}
...
export function generateConnectorPath(line) {
  const {
    from,
    to
  } = line;
  ...
  const cmds = []; // Calculate maximum corner radius

  const r1 = Math.min(CORNER_RADIUS, Math.abs(fromY - toY) / 2); // FROM
