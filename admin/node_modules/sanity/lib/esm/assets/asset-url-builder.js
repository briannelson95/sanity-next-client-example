...
export function assetUrlBuilder(url, options) {
  const {
    width,
    height,
    fit
  ...
  const params = [width && "w=".concat(width), height && "h=".concat(height), "fit=".concat(fit ? fit : defaultFit), 'q=85'].filter(Boolean).join('&');
