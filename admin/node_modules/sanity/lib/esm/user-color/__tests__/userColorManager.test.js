...
import { createUserColorManager } from '../';
...
describe('user color manager', () => {
  ...
  test('returns the same color as previously chosen if there are no unused', () => {
    const manager = createUserColorManager(options); // Ask for a color, but release it right away
  ...
  test('throws if current user color is not in colors list', () => {
    const incompleteColors = _omit(options.colors, ['blue']);

    expect(() => {
      createUserColorManager({ ...options,
  ...
  test('can return sync value', () => {
    expect(createUserColorManager(options).get('kokos')).toBe(colors === null || colors === void 0 ? void 0 : colors[colorPreferences.kokos]);
  });
});
