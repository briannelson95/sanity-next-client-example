...
export const intersectionObservableFor = isIntersectionObserverSupported() ? createIntersectionObserverBased() : createLegacyBased();
...
function createIntersectionObserverBased() {
  ...
  return function intersectionObservableFor(element) {
    ...
    }).pipe(mergeMap(() => intersectionObserverEntriesSubject.asObservable()), filter(entry => entry.target === element), map(ev => ({
...
function createLegacyBased() {
  ...
  return function intersectionObservableFor(element) {
    ...
    return merge(observableOf(isElementInViewport()), resize$, scroll$, orientationChange$).pipe( // @todo: consider "faking" more of the IntersectionObserverEntry api if possible
