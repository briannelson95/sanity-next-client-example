...
import { combineLatest, concat, defer, EMPTY, from, fromEvent, merge, of as observableOf, timer } from 'rxjs';
import { distinctUntilChanged, filter, map, mergeMap, publishReplay, refCount, share, startWith, switchMap, tap } from 'rxjs/operators';
...
export function create_preview_observeFields(context) {
  ...
  const getGlobalEvents = () => {
    if (!_globalListener) {
      const allEvents$ = from(versionedClient.listen('*[!(_id in path("_.**"))]', {}, {
        events: ['welcome', 'mutation'],
        includeResult: false,
        visibility: 'query',
        tag: 'preview.global'
      })).pipe(share()); // This is a stream of welcome events from the server, each telling us that we have established listener connection
      ...
      const welcome$ = allEvents$.pipe(filter(event => event.type === 'welcome'), publishReplay(1), refCount()); // This will keep the listener active forever and in turn reduce the number of initial fetches
  ...
  function listen(id) {
    ...
    return merge(globalEvents.welcome$, globalEvents.mutations$.pipe(filter(event => event.documentId === id)));
  }
  ...
  function fetchAllDocumentPathsWith(client, token) {
    ...
    return function fetchAllDocumentPath(selections) {
      ...
      return client.observable.fetch(toQuery(combinedSelections), {}, {
  ...
  function currentDatasetListenFields(id, fields) {
    return listen(id).pipe(switchMap(event => {
      if (event.type === 'welcome' || event.visibility === 'query') {
        return fetchDocumentPathsFast(id, fields).pipe(mergeMap(result => {
          return concat(observableOf(result), result === undefined // hack: if we get undefined as result here it can be because the document has
  ...
  const getBatchFetcherForDataset = _memoize(function getBatchFetcherForDataset(apiConfig, token) {
  ...
  function crossDatasetListenFields(id, fields, apiConfig) {
    ...
    }, ['token']).pipe(map(document => document === null || document === void 0 ? void 0 : document.token));
    return combineLatest([token$, visiblePoll$.pipe(startWith(0))]).pipe(switchMap(_ref => {
      let [token] = _ref;
  ...
  function createCachedFieldObserver(id, fields, apiConfig) {
    let latest = null;
    const changes$ = merge(defer(() => latest === null ? EMPTY : observableOf(latest)), apiConfig ? crossDatasetListenFields(id, fields, apiConfig) : currentDatasetListenFields(id, fields)).pipe(tap(v => latest = v), publishReplay(1), refCount());
    return {
      id,
      fields,
      changes$
  ...
  function cachedObserveFields(id, fields, apiConfig) {
    ...
    const missingFields = _difference(fields, _flatten(existingObservers.map(cachedFieldObserver => cachedFieldObserver.fields)));
    ...
    return combineLatest(cachedFieldObservers).pipe( // in the event that a document gets deleted, the cached values will be updated to store `undefined`
