...
export function validateWorkspaces(_ref) {
  let {
    workspaces
  } = _ref;
...
export function validateNames(workspaces) {
  ...
  workspaces.forEach((workspace, index) => {
    ...
    if (!rawName && !isSingleWorkspace) {
      throw new WorkspaceValidationError('All workspaces must have a `name`, unless only a single workspace is defined. ' + "Workspace ".concat(thisIdentifier, " did not define a `name`."), {
        workspace,
        index
      });
    }
    ...
    if (typeof name !== 'string') {
      throw new WorkspaceValidationError("Workspace at index ".concat(index, " defined an invalid `name` - must be a string."), {
        workspace,
        index
      });
    }
    ...
    if (existingWorkspace) {
      ...
      throw new WorkspaceValidationError("`name`s must be unique. Workspace ".concat(prevIdentifier, " and ") + "workspace ".concat(thisIdentifier, " both have the `name` `").concat(name, "`"), {
        workspace,
        index
      });
    }
    ...
    if (!/^[a-z0-9][a-z0-9_-]*$/i.test(name)) {
      throw new WorkspaceValidationError("All workspace `name`s must consist of only a-z, 0-9, underscore and dashes, " + "and cannot begin with an underscore or dash. " + "Workspace ".concat(thisIdentifier, " has the invalid name `").concat(name, "`"), {
        workspace,
        index
      });
    }
...
export function validateBasePaths(workspaces) {
  ...
  workspaces.forEach((workspace, index) => {
    ...
    if (existingWorkspace) {
      throw new WorkspaceValidationError("`basePath`s must be unique. Workspaces `".concat(existingWorkspace, "` and ") + "`".concat(getIdentifier(workspace, index), "` both have the `basePath` `").concat(basePath, "`"), {
        workspace,
        index
      });
    }
...
function hasBasePath(workspace, index) {
  ...
  if (typeof basePath === 'undefined') {
    throw new WorkspaceValidationError("If more than one workspace is defined, every workspace must have a `basePath` defined. " + "Workspace `".concat(name, "` is missing a `basePath`"), {
      workspace,
      index
    });
  }

  throw new WorkspaceValidationError("If more than one workspace is defined, every workspace must have a `basePath` defined. " + "Workspace `".concat(name, "` has an invalid `basePath` (must be a non-empty string)"), {
    workspace,
    index
  });
}
...
function validateBasePath(workspace, index) {
  ...
  if (!/^\/[a-z0-9/_-]*[a-z0-9_-]+$/i.test(basePath)) {
    throw new WorkspaceValidationError("All workspace `basePath`s must start with a leading `/`, " + "consist of only URL safe characters, " + "and cannot end with a trailing `/`. " + "Workspace `".concat(name, "`'s basePath is `").concat(basePath, "`"), {
      workspace,
      index
    });
  }
