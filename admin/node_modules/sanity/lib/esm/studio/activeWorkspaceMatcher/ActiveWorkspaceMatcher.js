...
export function ActiveWorkspaceMatcher(_ref) {
  let {
    children,
    LoadingComponent,
    NotFoundComponent,
  ...
  const [error, setError] = useState(null); // Throw error to closest error boundary
  ...
  useEffect(() => {
    ...
    const subscription = pathname$.pipe(map(pathname => matchWorkspace({
      pathname,
      workspaces
    }))).subscribe({
      error: setError,
  ...
  if (notFound) {
    return /*#__PURE__*/React.createElement(NotFoundComponent, {
  ...
  return /*#__PURE__*/React.createElement(ActiveWorkspaceMatcherContext.Provider, {
    value: value
  }, children);
}