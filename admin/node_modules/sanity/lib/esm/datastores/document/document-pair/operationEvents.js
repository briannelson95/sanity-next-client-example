import { defer, asyncScheduler, merge, of, Subject, EMPTY } from 'rxjs';
import { catchError, filter, groupBy, last, map, mergeMap, mergeMapTo, share, switchMap, take, tap, throttleTime } from 'rxjs/operators';
...
const operationCalls$ = new Subject();
export function emitOperation(operationName, idPair, typeName, extraArgs) {
  operationCalls$.next({
    operationName,
    idPair,
    typeName,
...
export function getOperationEvents(ctx) {
  ...
  const result$ = operationCalls$.pipe(groupBy(op => op.idPair.publishedId), mergeMap(groups$ => groups$.pipe( // although it might look like a but, dropping pending async operations here is actually a feature
  ...
  switchMap(args => operationArgs(ctx, args.idPair, args.typeName).pipe(take(1), switchMap(operationArguments => {
    ...
    args,
    error: err
  })))))), share()); // this enables autocommit after patch operations

  const AUTOCOMMIT_INTERVAL = 1000;
  const autoCommit$ = result$.pipe(filter(result => result.type === 'success' && result.args.operationName === 'patch'), throttleTime(AUTOCOMMIT_INTERVAL, asyncScheduler, {
    leading: true,
    trailing: true
  }), tap(result => {
  ...
  function listener(idPair, typeName) {
    ...
    if (!ret) {
      ret = merge(result$, autoCommit$.pipe(mergeMapTo(EMPTY))).pipe(filter(result => result.args.idPair.publishedId === idPair.publishedId), map(result => {
        const {
          operationName,
          idPair: documentIds
        } = result.args;
