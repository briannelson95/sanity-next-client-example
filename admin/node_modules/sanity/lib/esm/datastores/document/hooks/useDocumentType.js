...
export function useDocumentType(documentId) {
  ...
  const [resolvedState, setDocumentType] = useState(isResolved ? SYNC_RESOLVED_STATE : LOADING_STATE); // Reset documentType when documentId changes. Note that we're using the referentially
  // stable LOADING_STATE in order to prevent double rendering on initial load.

  useEffect(() => setDocumentType(LOADING_STATE), [publishedId, specifiedType]); // Load the documentType from Content Lake, unless we're already in a resolved state
