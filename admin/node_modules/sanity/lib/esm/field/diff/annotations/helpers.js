...
function getDiffAt(diff, path) {
  ...
  if (isIndexSegment(segment) || isKeySegment(segment)) {
    ...
    if (diff.type !== 'array') {
      warn("Failed to get item ".concat(location, " at path ").concat(pathToString(parentPath), " (not an array)"));
      return undefined;
    }

    const itemDiff = diff.items.find(isIndexSegment(segment) ? item => item.toIndex === segment : item => itemMatchesKey(item, segment));

    if (!itemDiff) {
      warn("Failed to get item ".concat(location, " at path ").concat(pathToString(parentPath), " (item missing)"));
      return undefined;
    }
  ...
  if (diff.type !== 'object') {
    warn("Failed to get property ".concat(segment, " at path ").concat(pathToString(parentPath), " (not an object)"));
    return undefined;
  }
  ...
  if (typeof fieldDiff === 'undefined') {
    warn("Failed to get property ".concat(segment, " at path ").concat(pathToString(parentPath), " (field did not exist)"));
    return undefined;
  }
 ...
 */
export function visitDiff(diff, visitor) {
...
function visitArrayDiff(diff, visitor, path) {
  ...
  diff.items.forEach(itemDiff => {
    ...
    const _key = itemDiff.diff.type === 'object' && ((_itemDiff$diff$toValu = itemDiff.diff.toValue) === null || _itemDiff$diff$toValu === void 0 ? void 0 : _itemDiff$diff$toValu._key);
