...
function buildUnsetPatches(rootDiff, paths) {
  ...
  return [{
    unset: [...new Set(patches.map(pathToString))]
  }];
}
/**
...
function buildMovePatches(itemDiff, parentDiff, path) {
  ...
  if (fromIndex === 0) {
    ...
    insertLocation = {
      before: pathToString([...basePath, 0])
    };
  } else {
    ...
    insertLocation = {
      after: pathToString([...basePath, prevSegment])
    };
  }
...
function buildUndoPatches(diff, rootDiff, path) {
  ...
  const inserts = patches.filter(patch => patch.op === 'insert').map(_ref => {
    ...
    return {
      insert: {
        after: pathToString(after),
        items
      }
    };
  });
  const unsets = patches.filter(patch => patch.op === 'unset').reduce((acc, patch) => acc.concat(pathToString(patch.path)), []);
...
function getParentStubs(path, rootDiff, stubbed) {
  ...
  for (let i = 1; i <= path.length; i++) {
    ...
    if (nextIsArrayElement && Array.isArray(itemValue) && !getValueAtPath(nextValue, path.slice(0, i + 1))) {
      ...
      const prevSeg = isKeyedObject(prevItem) ? {
        _key: prevItem._key
      } : indexAtPrev - 1;
...
function isStub(item, path, ignorePaths) {
  const isIgnoredPath = ignorePaths === null || ignorePaths === void 0 ? void 0 : ignorePaths.some(ignorePath => pathToString(ignorePath) === pathToString(path));
  const isEmptyArray = Array.isArray(item) && item.length <= 0;
  return isIgnoredPath || item === undefined || item === null || isEmptyArray || isEmptyObject(item) || onlyContainsStubs(item, path, ignorePaths);
}
