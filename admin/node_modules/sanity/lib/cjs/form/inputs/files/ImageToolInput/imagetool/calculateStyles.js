...
function calculateStyles() {
  ...
  const result = calculateHotSpotCrop(imageAspect, {
    hotspot,
    crop
  }, {
...
function calculateHotSpotCrop(sourceAspect, descriptor, spec) {
  ...
  const alignment = spec.align; // The rational aspect of the cropped image

  const netWidth = 1.0 - crop.left - crop.right;
  const netHeight = 1.0 - crop.top - crop.bottom; // Places the image inside the crop box
  ...
  const cropRationalAspect = netWidth / netHeight; // cropAspect is the real aspect ratio of the crop box in pixel-space

  const cropAspect = cropRationalAspect * sourceAspect; // Now we transform the hotspot to be expressed in ratios of the cropped area, not the
  // full image:
  ...
  const maxHotspotXScale = 1.0 / hotspot.width; // The scale at which the hotspot would fill the veiwport exactly in the Y direction

  const maxHotspotYScale = 1.0 / hotspot.height * cropAspect / viewportAspect; // This is the largest scale the image can have while still not cropping the hotspot:

  const maxScale = Math.min(maxHotspotXScale, maxHotspotYScale); // Now lets find the minimum scale we can have while maintaining full bleed (no letterboxing)
  ...
  let method;
  let outCrop; // Do we have to letterbox this image in order to leave the hotspot area uncropped?
  ...
  if (minFullBleedScale > maxScale) {
    ...
    method = 'letterbox';
    let letterboxScale;
    const diff = minFullBleedScale - maxScale; // Determine a scale where the image fills one dimension of the container
    ...
    const height = minFullBleedScale / cropAspect * viewportAspect; // Clamp top so that we will not move the image off of the viewport
