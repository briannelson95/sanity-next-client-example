...
function getAnnotationAtPath(diff, diffPath) {
  const path = Array.isArray(diffPath) ? diffPath : (0, _helpers.stringToPath)(diffPath);
...
function getDiffAt(diff, path) {
  ...
  if ((0, _types.isIndexSegment)(segment) || (0, _types.isKeySegment)(segment)) {
    const location = (0, _types.isIndexSegment)(segment) ? "at index ".concat(segment) : "with key ".concat(segment._key);
    ...
    const itemDiff = diff.items.find((0, _types.isIndexSegment)(segment) ? item => item.toIndex === segment : item => itemMatchesKey(item, segment));
