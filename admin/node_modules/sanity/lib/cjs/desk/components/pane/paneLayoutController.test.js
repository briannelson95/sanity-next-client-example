...
describe('paneLayoutController', () => {
  it('should emit state changes', () => {
    const ctrl = (0, _paneLayoutController.createPaneLayoutController)(); // Subscribe to state changes

    const setStateFn = jest.fn();
    ctrl.subscribe(setStateFn); // Set root element (layout containing panes)

    const rootElement = document.createElement('div');
    ctrl.setRootElement(rootElement); // Mount pane #1

    const pane1Element = document.createElement('div');
    rootElement.appendChild(pane1Element);
    ctrl.mount(pane1Element, {
      flex: 1,
      id: 'pane-1',
      minWidth: 100
    }); // Mount pane #2
    ...
    expect(setStateFn.mock.calls.length).toBe(1); // Resize the layout
