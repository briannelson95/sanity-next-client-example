...
const PathSegment = _styledComponents.default.span(_templateObject || (_templateObject = _taggedTemplateLiteral(["\n  &:not(:last-child)::after {\n    content: ' \u279D ';\n    opacity: 0.5;\n  }\n"])));
...
function StructureError(_ref) {
  ...
  if (!(error instanceof _structureResolvers.PaneResolutionError)) {
    throw error;
  }
  ...
  const showStack = stack && !(cause instanceof _structureBuilder.SerializeError) && !error.message.includes('Module build failed:');
  const path = cause instanceof _structureBuilder.SerializeError ? cause.path : [];
  const helpId = cause instanceof _structureBuilder.SerializeError && cause.helpId || error.helpId;
