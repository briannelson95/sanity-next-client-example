...
const progress = url => {
  return new _rxjs.Observable(observer => {
    ...
    function onError(error) {
      ...
      (0, _debug.debug)("Error received: ".concat(error));
    ...
    function onMessage(event) {
      ...
      if (data.state === 'failed') {
        (0, _debug.debug)('Job failed. Data: %o', event);
        observer.error(event);
      } else if (data.state === 'completed') {
        (0, _debug.debug)('Job succeeded. Data: %o', event);
        onComplete();
      } else {
        (0, _debug.debug)("Job progressed. Data: %o", event);
        observer.next(data);
      }
    }
...
const followProgress = (jobId, client, output) => {
  ...
  (0, _debug.debug)("Listening to ".concat(listenUrl));
...
const copyDatasetCommand = {
  ...
  action: async (args, context) => {
    ...
    const nameError = sourceDataset && (0, _validateDatasetName.validateDatasetName)(sourceDataset);
    ...
    const sourceDatasetName = await (sourceDataset || (0, _datasetNamePrompt.promptForDatasetName)(prompt, {
    ...
    const targetDatasetName = await (targetDataset || (0, _datasetNamePrompt.promptForDatasetName)(prompt, {
    ...
    const err = (0, _validateDatasetName.validateDatasetName)(targetDatasetName);
...
var _default = copyDatasetCommand;
