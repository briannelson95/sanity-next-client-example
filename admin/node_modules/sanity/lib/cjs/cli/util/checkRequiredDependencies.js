...
async function checkRequiredDependencies(context) {
  ...
  const [studioPackageManifest, installedStyledComponentsVersion] = await Promise.all([await readPackageManifest(_path.default.join(studioPath, 'package.json'), defaultStudioManifestProps), await readModuleVersion(studioPath, 'styled-components')]);
...
function readPackageJson(filePath) {
  return (0, _promises.readFile)(filePath, 'utf8').then(res => JSON.parse(res));
}