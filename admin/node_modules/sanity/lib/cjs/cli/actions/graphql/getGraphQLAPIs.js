...
var _getStudioConfig = require("../../util/getStudioConfig");
...
if (!_worker_threads.isMainThread && _worker_threads.parentPort) {
  getGraphQLAPIsForked(_worker_threads.parentPort);
}
...
async function getGraphQLAPIs(cliContext) {
  ...
  const defaultSchema = (0, _schema.createSchema)({
    name: 'default',
    types: []
  });
...
async function resolveGraphQLApis(_ref2) {
  ...
  const workspaces = await (0, _getStudioConfig.getStudioConfig)({
    basePath: workDir
  });
  ...
  const hasGraphQLConfig = Boolean(cliConfig === null || cliConfig === void 0 ? void 0 : cliConfig.graphql);
  ...
  const apiDefs = validateCliConfig((cliConfig === null || cliConfig === void 0 ? void 0 : cliConfig.graphql) || []);
