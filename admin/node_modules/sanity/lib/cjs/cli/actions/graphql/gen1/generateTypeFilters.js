...
function generateTypeFilters(types) {
  const queryable = types.filter(_helpers.isNonUnion).filter(type => type.type === 'Object' && type.interfaces && type.interfaces.includes('Document'));
  return queryable.map(type => {
    const name = "".concat(type.name, "Filter");
    const fields = (0, _flatten2.default)(type.fields.map(createFieldFilters)).filter(Boolean);
    return {
      name,
      kind: 'InputObject',
      fields: [...fields, ...getDocumentFilters()]
    };
  });
}
