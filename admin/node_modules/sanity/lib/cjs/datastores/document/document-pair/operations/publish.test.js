...
jest.mock('../utils/isLiveEditEnabled', () => ({
  isLiveEditEnabled: jest.fn()
}));
beforeEach(() => {
  ;

  _isLiveEditEnabled.isLiveEditEnabled.mockClear();
});
describe('publish', () => {
  describe('disabled', () => {
    ...
    it('returns with LIVE_EDIT_ENABLED if isLiveEditEnabled', () => {
      ...
      _isLiveEditEnabled.isLiveEditEnabled.mockImplementation( // eslint-disable-next-line max-nested-callbacks
      () => true);

      expect(_publish.publish.disabled( // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    it('returns ALREADY_PUBLISHED if there is no draft and there is a published version', () => {
      ...
      expect(_publish.publish.disabled({
        typeName: 'blah',
        snapshots: {
          draft: undefined,
          published: {}
