...
class Timeline {
  ...
  addRemoteMutation(entry) {
    ...
    const transaction = pending ? pending.transaction : {
      index: 0,
  ...
  _addChunksFromTransactions() {
    ...
    const lastIdx = this._transactions.lastIdx; // Add transactions at the end:
    ...
    for (let idx = firstTransactionChunked - 1; idx >= firstIdx; idx--) {
      ...
      this._chunks.mergeAtBeginning((0, _chunker.chunkFromTransaction)(transaction), _chunker.mergeChunk);
    }
  }
  ...
  _createInitialChunk() {
    if (this.reachedEarliestEntry) {
      ...
      if (!firstTx) return;
      const initialChunk = (0, _chunker.chunkFromTransaction)(firstTx);
  ...
  chunkByTransactionIndex(idx) {
    let startChunkIdx = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  ...
  calculateDiff(initialDoc, finalDoc, firstIdx, lastIdx) {
    let draftValue = _mendoza.incremental.wrap(initialDoc.draft, null);

    let publishedValue = _mendoza.incremental.wrap(initialDoc.published, null);

    const initialValue = getValue(draftValue, publishedValue);
    const initialAttributes = (0, _utils.getAttrs)(initialDoc);
    let firstChunk = null; // Loop over all of the chunks:
    ...
    for (let chunkIdx = firstIdx; chunkIdx <= lastIdx; chunkIdx++) {
      ...
      for (let idx = chunk.start; idx < chunk.end; idx++) {
        ...
        const didHaveDriaft = _mendoza.incremental.getType(preDraftValue) !== 'null';
        const haveDraft = _mendoza.incremental.getType(draftValue) !== 'null';
        const havePublished = _mendoza.incremental.getType(publishedValue) !== 'null';
    ...
    const finalValue = _mendoza.incremental.getType(draftValue) === 'null' ? publishedValue : draftValue;
    const finalAttributes = (0, _utils.getAttrs)(finalDoc);
