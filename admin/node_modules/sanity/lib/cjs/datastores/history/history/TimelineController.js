...
var _remoteSnapshots = require("../../document/document-pair/remoteSnapshots");
...
class TimelineController {
  ...
  findRangeForNewSince(since) {
    const revChunk = this.revTime; // If the the `since` timestamp is earlier than the `rev`, then we can
  ...
  async tick() {
    const shouldFetchMore = this._aligner.acceptsHistory && !this.timeline.reachedEarliestEntry && (this.selectionState === 'loading' || this._fetchMore || this.timeline.chunkCount <= this._fetchAtLeast);
  ...
  async fetchMoreTransactions() {
    ...
    const url = "/data/history/".concat(dataset, "/transactions/").concat(publishedId, ",").concat(draftId, "?").concat(queryParams);
    const stream = await (0, _getJsonStream.getJsonStream)(this.client.getUrl(url), token);
...
function createObservableController(options) {
  return new _rxjs.Observable(observer => {
    const controller = new TimelineController({ ...options,
    ...
    return (0, _remoteSnapshots.remoteSnapshots)(options.client, {
