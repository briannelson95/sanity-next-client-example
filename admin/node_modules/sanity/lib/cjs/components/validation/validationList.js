...
function ValidationList(props) {
  ...
  const errors = validation.filter(_types.isValidationErrorMarker);
  const warnings = validation.filter(_types.isValidationWarningMarker);
  const info = validation.filter(_types.isValidationInfoMarker);
