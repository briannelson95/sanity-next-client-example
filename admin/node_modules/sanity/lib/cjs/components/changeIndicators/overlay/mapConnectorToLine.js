...
function getConnectorLinePoint(rect, bounds) {
  ...
  const isAbove = rect.top + rect.height < bounds.top + _constants.ARROW_MARGIN_Y;
  const isBelow = rect.top > bounds.top + bounds.height - _constants.ARROW_MARGIN_Y;
  return {
    bounds: bounds,
    left: rect.left,
    top: centerY,
...
function mapConnectorToLine(connector) {
  ...
  const maxStartY = Math.max(to.startY, from.startY); // Align from <-> to vertically
