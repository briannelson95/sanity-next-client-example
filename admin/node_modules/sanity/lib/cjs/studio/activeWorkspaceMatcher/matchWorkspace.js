...
function matchWorkspace(_ref) {
  ...
  for (const {
    ...
    if (new RegExp("^".concat((0, _escapeRegExp2.default)(basePath), "(\\/|$)"), 'i').test(pathname) || basePath === '/') {
      return {
        type: 'match',
        workspace
      };
  ...
  function createCommonBasePathRegex(_ref2) {
    ...
    if (!first) return '';
    return "(\\/".concat((0, _escapeRegExp2.default)(first)).concat(createCommonBasePathRegex(rest), "(\\/|$))?");
  }
