...
export default function PortableText(props) {
    ...
    const renderChild = useCallback((ptDiffChild) => {
        ...
        if (spanSchemaType) {
            ...
            const segments = (childrenDiff.items[0].diff &&
            ...
            if (isEmptyTextChange(block, diff) &&
                ...
                if (textDiff && textDiff.action !== 'unchanged') {
                    return (React.createElement(DiffCard, { annotation: textDiff.annotation, as: textDiff.action === 'removed' ? 'del' : 'ins', key: `empty-block-${block._key}`, tooltip: {
                            description: `${startCase(textDiff.action)} empty text`,
                        } },
                        React.createElement("span", null, TextSymbols.EMPTY_BLOCK_SYMBOL)));
                }
            }
            ...
            segments.forEach((seg) => {
                ...
                if (isChildStart) {
                    ...
                    if (key) {
                        ...
                        returnedChildren.push(React.createElement(InlineObject, { key: `inline-object-${originChild._key}`, object: originChild, path: [{ _key: block._key }, 'children', { _key: originChild._key }], diff: objectDiff, schemaType: objectSchemaType }));
                    }
                }
                else if (seg.text) {
                    // TODO: find a better way of getting a removed child
                    const getChildFromFromValue = () => diff.origin.fromValue?.children.find((cld) => cld.text && cld.text.match(escapeRegExp(seg.text)));
                    ...
                    const textDiff = childDiff?.fields?.text
                        ...
                        : undefined;
                    const text = (React.createElement(Text, { diff: textDiff, key: `text-${child._key}-${segIndex}`, path: [{ _key: block._key }, 'children', { _key: child._key }], childDiff: childDiff, segment: seg }, renderTextSegment({
                        diff,
                        child,
                        decoratorTypes,
                        seg,
                    ...
                    if (endedAnnotation) {
                        ...
                        if (lastChild && lastChild.key !== key) {
                            ...
                            const objectSchemaType = endedAnnotation &&
                                spanSchemaType.annotations &&
                                spanSchemaType.annotations.find((type) => endedAnnotation &&
                                    endedAnnotation.object &&
                                    type.name === endedAnnotation.object._type);
                            returnedChildren.push(React.createElement(Annotation, { object: endedAnnotation.object, diff: annotationDiff, path: [{ _key: block._key }, 'children', { _key: child._key }], schemaType: objectSchemaType, key: key },
                                React.createElement(React.Fragment, null, annotationSegments[endedAnnotation.mark])));
                        }
    ...
    return (React.createElement(Block, { block: diff.displayValue, diff: diff }, React.createElement(React.Fragment, null, (diff.displayValue.children || []).map((child) => renderChild(child)))));
}
function renderTextSegment({ diff, child, decoratorTypes, seg, segIndex, spanSchemaType, }) {
    // Newlines
    if (seg.text === '\n') {
        return React.createElement("br", null);
    }
    // Make sure we render trailing spaces correctly
    let children = (React.createElement("span", { key: `text-${segIndex}` }, seg.text.replace(/ /g, TextSymbols.TRAILING_SPACE_SYMBOL)));
    ...
    if (activeMarks && activeMarks.length > 0) {
        activeMarks.forEach((mark) => {
            if (isDecorator(mark, spanSchemaType)) {
                ...
                React.createElement(Decorator, { mark: mark, key: `decorator-${mark}-${child._key}-${segIndex}` }, children));
            }
        });
    }
    return children;
}
function renderDecorators({ activeMarks, decoratorTypes, diff, children, seg, segIndex, spanDiff, spanSchemaType, }) {
    let returned = React.createElement("span", { key: `text-segment-${segIndex}` }, children);
    ...
    const marksDiff = spanDiff?.fields?.marks;
    const marksAnnotation = (marksDiff && marksDiff.action !== 'unchanged' && marksDiff.annotation) ||
    ...
    matches.forEach((match) => {
        ...
        if (set) {
            ...
            const mark = decoratorTypes[isMarkStart ? decoratorSymbolsStart.indexOf(sym) : decoratorSymbolsEnd.indexOf(sym)]?.value || sym; // Annotation marks are uniqe anyway
    ...
    if (marksAnnotation &&
        ...
        returned = (React.createElement(DiffCard, { annotation: marksAnnotation, key: `diffcard-annotation-${segIndex}-${marksChanged.join('-')}`, as: 'ins', tooltip: {
