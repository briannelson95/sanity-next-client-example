...
`;
export function Annotation({ children, diff, object, schemaType, path, ...restProps }) {
    if (!schemaType) {
        return React.createElement(AnnotationWrapper, { ...restProps }, "Unknown schema type");
    }
    if (diff && diff.action !== 'unchanged') {
        return (React.createElement(AnnnotationWithDiff, { ...restProps, diff: diff, object: object, schemaType: schemaType, path: path }, children));
    }
    return React.createElement(AnnotationWrapper, null, children);
}
function AnnnotationWithDiff({ diff, children, object, schemaType, path, ...restProps }) {
    ...
    const markDefPath = useMemo(() => [path[0]].concat(['markDefs', { _key: object._key }]), [object._key, path]);
    ...
    const isEditing = useMemo(() => values.filter(([p]) => p.startsWith(myValue)).length > 0, [myValue, values]);
    useEffect(() => {
        if (!open && isEditing) {
            setOpen(true);
            onSetFocus(myPath);
    ...
    const popoverContent = (React.createElement(DiffContext.Provider, { value: { path: myPath } },
        React.createElement(PopoverContainer, { padding: 3 },
    ...
    return (React.createElement(AnnotationWrapper, { ...restProps, onClick: handleOpenPopup, style: style, "data-changed": "", "data-removed": diff.action === 'removed' ? '' : undefined },
        React.createElement(Popover, { content: popoverContent, open: open, ref: setPopoverElement, portal: true },
            React.createElement(PreviewContainer, { paddingLeft: 1 },
                React.createElement(DiffTooltip, { annotations: annotations, description: `${diff.action} annotation` },
                    React.createElement(InlineBox, { style: { display: 'inline-flex' } },
