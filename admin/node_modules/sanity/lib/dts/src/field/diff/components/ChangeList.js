...
export function ChangeList({ diff, fields, schemaType }) {
    ...
    const allChanges = useMemo(() => buildObjectChangeList(schemaType, diff, path, [], { fieldFilter: fields }), [schemaType, fields, path, diff]);
    ...
    if (changes.length === 0) {
        return isRoot ? React.createElement(NoChanges, null) : null;
    }
    const showFooter = isRoot && changes.length > 1;
    return (React.createElement(Card, null,
        React.createElement(Stack, { space: 5 },
            React.createElement(Stack, { as: ChangeListWrapper, space: 5 }, changes.map((change) => (React.createElement("div", { key: change.key },
                React.createElement(ChangeResolver, { change: change, key: change.key, "data-revert-all-changes-hover": confirmRevertAllHover ? '' : undefined, readOnly: isReadOnly || change?.readOnly, hidden: change?.hidden }))))),
            showFooter && isComparingCurrent && !isPermissionsLoading && permissions?.granted && (React.createElement(PopoverWrapper, { content: React.createElement(Box, null,
                    "Are you sure you want to revert all ",
                    changes.length,
                    " changes?",
                    React.createElement(Grid, { columns: 2, gap: 2, marginTop: 2 },
                        React.createElement(Button, { mode: "ghost", text: "Cancel", onClick: closeRevertAllChangesConfirmDialog }),
                        React.createElement(Button, { tone: "critical", text: "Revert all", onClick: revertAllChanges }))), open: confirmRevertAllOpen, padding: 4, placement: 'left', portal: true, ref: setRevertAllContainerElement },
                React.createElement(Stack, null,
                    React.createElement(Button, { tone: "critical", mode: "ghost", text: "Revert all changes", icon: RevertIcon, onClick: handleRevertAllChangesClick, onMouseEnter: handleRevertAllChangesMouseEnter, onMouseLeave: handleRevertAllChangesMouseLeave, disabled: isReadOnly })))))));
}
