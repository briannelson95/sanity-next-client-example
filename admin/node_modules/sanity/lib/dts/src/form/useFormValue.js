...
export function useFormValue(path) {
    ...
    useEffect(() => patchChannel.subscribe(() => documentValueSubject.next(getDocument())), [documentValueSubject, getDocument, patchChannel]);
    // Subscribe to value at `path`
    useEffect(() => {
        const value$ = documentValue$.pipe(map((documentValue) => uniquePath.length === 0 ? documentValue : getValueAtPath(documentValue, uniquePath)), distinctUntilChanged((x, y) => isEqual(x, y)));
        const sub = value$.subscribe(setValue);
        return () => sub.unsubscribe();
    }, [documentValue$, uniquePath]);
    return value;
}
//# sourceMappingURL=useFormValue.js.map