...
export function getReferenceInfo(documentPreviewStore, id, referenceType) {
    ...
    return pairAvailability$.pipe(switchMap((pairAvailability) => {
        if (!pairAvailability.draft.available && !pairAvailability.published.available) {
            ...
            const availability = pairAvailability.draft.reason === 'PERMISSION_DENIED' ||
        ...
        return typeName$.pipe(switchMap((typeName) => {
            ...
            const publishedPreview$ = documentPreviewStore
                ...
                : undefined), startWith(undefined));
            const value$ = combineLatest([draftPreview$, publishedPreview$]).pipe(map(([draft, published]) => ({ draft, published })));
...
export function referenceSearch(client, textTerm, type, options) {
    ...
    return searchWeighted(textTerm, { includeDrafts: true }).pipe(map((results) => results.map((result) => result.hit)), map(collate), 
