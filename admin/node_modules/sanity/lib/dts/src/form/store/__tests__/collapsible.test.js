...
test("doesn't make primitive fields collapsed even if they are configured to be", () => {
    ...
    const primitiveFields = result.members.filter((member) => member.kind === 'field' &&
...
function getDeepFieldMember(objectFormNode, path) {
    ...
    if (!nextField) {
        throw new Error(`Field with name "${head}" not found for object node at path "${pathToString(objectFormNode.path)}"`);
    }
    if (tail.length === 0) {
        return nextField;
    }
