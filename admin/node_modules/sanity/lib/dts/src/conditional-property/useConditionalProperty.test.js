...
import { unstable_useConditionalProperty as useConditionalProperty, } from './useConditionalProperty';
const dummyDocument = {
...
describe('Conditional property resolver', () => {
    ...
    it('calls callback function', async () => {
        ...
        const { result } = renderHook(() => useConditionalProperty({
    ...
    it('returns true when the current user does not have role "developer"', async () => {
        const TestWrapper = await createTestProvider();
        const { result } = renderHook(() => useConditionalProperty({
            checkProperty: jest.fn(({ currentUser }) => !currentUser?.roles.some((role) => role.name === 'developer')),
            ...DEFAULT_PROPS,
            value: 'test value',
        }), { wrapper: TestWrapper });
        expect(result.current).toBeTruthy();
    ...
    it('returns true when the current user has role "administrator"', async () => {
        const TestWrapper = await createTestProvider();
        const { result } = renderHook(() => useConditionalProperty({
            checkProperty: jest.fn(({ currentUser }) => Boolean(currentUser?.roles.some((role) => role.name === 'administrator'))),
            ...DEFAULT_PROPS,
            value: 'test value',
        }), { wrapper: TestWrapper });
        expect(result.current).toBeTruthy();
...
//# sourceMappingURL=useConditionalProperty.test.js.map