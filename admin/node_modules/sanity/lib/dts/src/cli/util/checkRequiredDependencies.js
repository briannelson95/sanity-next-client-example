...
export async function checkRequiredDependencies(context) {
    ...
    // (^x.x.x / ~x.x.x / x.x.x)
    if (isComparableRange(declaredStyledComponentsVersion) &&
...
async function installDependencies(dependencies, context) {
    ...
    if (mostOptimal && pkgManager !== mostOptimal) {
        output.warn(`WARN: This project appears to be installed with or using ${mostOptimal} - using a different package manager _may_ result in errors.`);
    }
