...
export function extractFromSanitySchema(sanitySchema, extractOptions = {}) {
    ...
    const hasErrors = sanitySchema._validation &&
        sanitySchema._validation.some((group) => group.problems.some((problem) => problem.severity === 'error'));
    ...
    const types = typeNames
        ...
        .filter((schemaType) => Boolean(schemaType))
        .filter(isBaseType)
    ...
    function isArrayType(type) {
        return Boolean(('jsonType' in type && type.jsonType === 'array') ||
    ...
    function getObjectDefinition(def, parent) {
        ...
        return {
            ...
            fields: objectFields.map((field) => isArrayOfBlocks(field)
    ...
    function hasValidationFlag(field, flag) {
        return ('validation' in field &&
            ...
            field.validation.some((rule) => rule && '_rules' in rule && rule._rules.some((item) => item.flag === flag)));
    }
