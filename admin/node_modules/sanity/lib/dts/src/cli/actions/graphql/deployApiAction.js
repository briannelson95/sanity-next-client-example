...
export default async function deployGraphQLApiAction(args, context) {
    ...
    for (const apiDef of apiDefs) {
        ...
        const { currentGeneration, playgroundEnabled } = await getCurrentSchemaProps(projectClient, dataset, tag);
...
async function shouldEnablePlayground({ dryRun, spinner, playgroundConfiguration, playgroundCurrentlyEnabled, prompt, }) {
...
async function resolveApiGeneration({ currentGeneration, specifiedGeneration, index, force, output, prompt, }) {
