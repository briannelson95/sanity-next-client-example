...
export function validateStructure(structure) {
    ...
    if (!isStructure(structure)) {
        throw new Error(`Structure needs to export a function, an observable, a promise or a structure builder, got ${typeof structure}`);
    }
    return structure;
}
...
function isStructure(structure) {
    ...
    if (!isRecord(structure))
        return false;
    return (typeof structure.serialize !== 'function' ||
...
function warnOnUnknownExports(mod) {
    ...
    for (const key of unknownKeys) {
        const { closest } = known.reduce((acc, current) => {
...
//# sourceMappingURL=validateStructure.js.map