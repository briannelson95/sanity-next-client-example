...
const wrapWithPublishReplay = (next) => (...args) => {
...
export function createPaneResolver(middleware) {
    ...
    const resolvePane = rethrowWithPaneResolutionErrors(wrapWithPublishReplay(middleware((unresolvedPane, context, flatIndex) => {
        ...
        if (isSubscribable(unresolvedPane)) {
            return observableFrom(unresolvedPane).pipe(switchMap((result) => resolvePane(result, context, flatIndex)));
        }
...
//# sourceMappingURL=createPaneResolver.js.map