    ...
    try {
        ...
        switchMap((paneNode) => {
            ...
            next?.routerPaneSibling.id.startsWith('__edit__')) {
                ...
                nextStream = resolvePaneTree({
                    unresolvedPane: typeof paneNode.child === 'function'
                        ? memoBind(paneNode, 'child')
                        : paneNode.child,
                    flattenedRouterPanes: rest,
                    parent: paneNode,
                    path: context.path,
        ...
        if (e instanceof PaneResolutionError) {
            if (e.context) {
                console.warn(`Pane resolution error at index ${e.context.index}${e.context.splitIndex > 0 ? ` for split pane index ${e.context.splitIndex}` : ''}: ${e.message}${e.helpId ? ` - see ${generateHelpUrl(e.helpId)}` : ''}`, e);
            }
 ...
 */
export function createResolvedPaneNodeStream({ routerPanesStream, rootPaneNode, initialCacheState = {
    ...
    map((routerPanes) => {
        const flattenedRouterPanes = routerPanes
            .flatMap((routerPaneGroup, groupIndex) => routerPaneGroup.map((routerPaneSibling, siblingIndex) => ({
            routerPaneSibling,
            groupIndex,
            siblingIndex,
        })))
    ...
    switchMap(({ flattenedRouterPanes, resolvePane }) => resolvePaneTree({
...
//# sourceMappingURL=createResolvedPaneNodeStream.js.map