...
export default function ResizeStory() {
    ...
    return (React.createElement(PaneLayoutContext.Provider, { value: paneLayout },
        React.createElement(PaneLayoutRoot, { "data-resizing": state.resizing ? '' : undefined, height: "fill", ref: setRootElement },
            React.createElement(Pane, { currentMinWidth: undefined, 
                // currentMaxWidth={320}
            ...
            React.createElement(Pane, { currentMinWidth: undefined, 
                // currentMaxWidth={320}
                flex: 1, minWidth: 160, maxWidth: 320 }),
            React.createElement(Pane, { currentMinWidth: 320, currentMaxWidth: undefined, flex: 2, minWidth: 160, maxWidth: undefined }))));
}
function Pane(props) {
    const { currentMinWidth: currentMinWidthProp, currentMaxWidth: currentMaxWidthProp, flex: flexProp = 1, minWidth: minWidthProp, maxWidth: maxWidthProp, } = props;
    const [rootElement, setRootElement] = useState(null);
    ...
    return (React.createElement(PaneView, { collapsed: collapsed, currentMinWidth: currentMinWidth, currentMaxWidth: currentMaxWidth, expanded: expanded, flex: flex, index: paneIndex, isLast: isLast, minWidth: minWidth, maxWidth: maxWidth, nextCollapsed: nextCollapsed, onCollapse: handleCollapse, onExpand: handleExpand, resizing: resizing, rootElement: rootElement, setRootElement: setRootElement }));
}
...
`;
const PaneView = memo(function PaneView(props) {
    const { collapsed, currentMinWidth, currentMaxWidth, expanded, flex, index, isLast, minWidth, maxWidth, nextCollapsed, onCollapse, onExpand, resizing, rootElement, setRootElement, } = props;
    const style = useMemo(() => ({
        flex,
        minWidth,
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(PaneViewRoot, { "data-as": "button", "data-resizing": resizing ? '' : undefined, id: `pane-${index}`, onClick: handleClick, overflow: "hidden", ref: setRootElement, style: style, tone: expanded ? 'primary' : undefined }, !collapsed && (React.createElement(Box, { padding: 4 },
            React.createElement(Code, { size: 1 }, [
                `#${index}`,
                `collapsed=${collapsed}`,
        ...
        !isLast && React.createElement(PaneDivider, { disabled: collapsed || nextCollapsed, element: rootElement })));
});
//# sourceMappingURL=ResizeStory.js.map