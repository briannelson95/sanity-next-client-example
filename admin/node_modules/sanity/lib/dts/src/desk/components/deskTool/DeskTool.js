 ...
 * @internal
 */
export const DeskTool = memo(function DeskTool({ onPaneChange }) {
    ...
    if (!hasDefinedDocumentTypes) {
        return React.createElement(NoDocumentTypesScreen, null);
    }
    return (React.createElement(PortalProvider, { element: portalElement || null },
        React.createElement(StyledPaneLayout, { flex: 1, height: layoutCollapsed ? undefined : 'fill', minWidth: 512, onCollapse: handleRootCollapse, onExpand: handleRootExpand }, paneDataItems.map(({ active, childItemId, groupIndex, itemId, key: paneKey, pane, index: paneIndex, params: paneParams, path, payload, siblingIndex, selected, }) => (React.createElement(Fragment, { key: `${pane === LOADING_PANE ? 'loading' : pane.type}-${paneIndex}` }, pane === LOADING_PANE ? (React.createElement(LoadingPane, { paneKey: paneKey, path: path, selected: selected })) : (React.createElement(DeskToolPane, { active: active, groupIndex: groupIndex, index: paneIndex, pane: pane, childItemId: childItemId, itemId: itemId, paneKey: paneKey, params: paneParams, payload: payload, selected: selected, siblingIndex: siblingIndex })))))),
        React.createElement("div", { "data-portal": "", ref: setPortalElement })));
});
//# sourceMappingURL=DeskTool.js.map