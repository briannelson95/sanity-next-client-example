...
export function createSettingsStore() {
    ...
    const updates$ = set$.pipe(switchMap((event) => storageBackend.set(event.key, event.value).pipe(map((nextValue) => ({
        key: event.key,
        value: nextValue,
    })))));
    const listen = (key, defValue) => {
        return merge(storageBackend.get(key, defValue), updates$.pipe(filter((update) => update.key === key), map((update) => update.value)));
    };
    const set = (key, value) => {
        set$.next({ key, value });
    };
