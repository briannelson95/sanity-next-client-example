...
export function createBroadcastChannel(namespace) {
    const storageEvents$ = typeof window === 'undefined'
        ? of() // No storage events in non-browser environments
    ...
    const messages$ = merge(broadcastedMessages$, storageEvents$.pipe(filter((event) => event.key === storageKey), map((event) => event.newValue), filter(isNonNullable), map((newValue) => JSON.parse(newValue)))).pipe(
...
//# sourceMappingURL=createBroadcastChannel.js.map