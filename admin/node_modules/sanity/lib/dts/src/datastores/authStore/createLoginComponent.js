...
async function getProviders({ client, mode, providers: customProviders = [] }) {
    const { providers, thirdPartyLogin, sso } = await client.request({
        uri: '/auth/providers',
    });
...
function createHrefForProvider({ loginMethod, projectId, url, basePath, }) {
...
export function createLoginComponent({ getClient, loginMethod, redirectOnSingle, ...providerOptions }) {
    const useClient = createHookFromObservableFactory(getClient);
    function LoginComponent({ projectId, basePath }) {
        ...
        const redirectUrl = redirectOnSingle &&
            providers?.length === 1 &&
            providers?.[0] &&
        ...
        if (loading) {
            return (React.createElement(Flex, { align: "center", direction: "column", gap: 4, justify: "center", padding: 6, sizing: "border" },
                React.createElement(Text, { muted: true }, "Loading\u2026"),
                React.createElement(Spinner, { muted: true })));
        }
        return (React.createElement(Stack, { space: 4 },
            React.createElement(Heading, { align: "center", size: 1 }, "Choose login provider"),
            React.createElement(Stack, { space: 2 }, providers.map((provider, index) => (React.createElement(Button
            // eslint-disable-next-line react/no-array-index-key
            , { 
                // eslint-disable-next-line react/no-array-index-key
                icon: providerLogos[provider.name], key: `${provider.url}_${index}`, as: "a", href: createHrefForProvider({
                    loginMethod,
                    projectId,
                    url: provider.url,
                    basePath,
                }), mode: "ghost", text: provider.title }))))));
    }
    return LoginComponent;
}
//# sourceMappingURL=createLoginComponent.js.map