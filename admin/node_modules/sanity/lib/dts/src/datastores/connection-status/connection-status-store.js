...
export { onRetry };
const createErrorStatus = ({ error, isOffline, attemptNo, retryAt, }) => ({
    type: 'error',
    error,
    attemptNo,
    isOffline,
 ...
 */
export function createConnectionStatusStore({ bifur, }) {
    const connectionStatus$ = merge(bifur.heartbeats, onOffline$.pipe(mergeMapTo(throwError(new Error('The browser went offline'))))).pipe(map((ts) => ({ type: 'connected', lastHeartbeat: ts })), catchWithCount((error, successiveErrorsCount, caught) => {
...
//# sourceMappingURL=connection-status-store.js.map