...
test('should return correct document type on document ID transition', async () => {
    ...
    let documentId = 'grrm';
    const { result, rerender, waitForNextUpdate } = renderHook(() => useDocumentType(documentId, undefined), { wrapper: await createWrapperComponent(client) });
    // First lookup (author)
    expect(result.current).toEqual({ isLoaded: false, documentType: undefined });
    await waitForNextUpdate();
    expect(result.current).toEqual({ isLoaded: true, documentType: 'author' });
...
test('should return correct document type when transitioning from undefined type to specified type', async () => {
    ...
    const { result, rerender, waitForNextUpdate } = renderHook(() => useDocumentType('grrm', documentType), { wrapper: await createWrapperComponent(client) });
...
test('should cancel ongoing requests when transitioning document ID', async () => {
    ...
    const responseDelayedGrrm = defer(() => of(['person']).pipe(observeOn(asyncScheduler), delay(5000), tap(hasResolvedFirst)));
    const responseGot = defer(() => of(['book']).pipe(observeOn(asyncScheduler)));
...
//# sourceMappingURL=useDocumentType.test.js.map