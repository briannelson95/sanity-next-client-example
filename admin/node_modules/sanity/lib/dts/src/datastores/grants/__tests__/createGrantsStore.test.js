...
describe('checkDocumentPermission', () => {
    it('takes in a permission and document and returns an observable of PermissionCheckResult', async () => {
        ...
        await expect(checkDocumentPermission('create', { _id: 'example-id', _type: 'book' })
            .pipe(first())
            .toPromise()).resolves.toEqual({
            granted: false,
            reason: 'No matching grants found',
        });
        await expect(checkDocumentPermission('read', { _id: 'example-id', _type: 'book' }).pipe(first()).toPromise()).resolves.toEqual({
            granted: true,
...
//# sourceMappingURL=createGrantsStore.test.js.map