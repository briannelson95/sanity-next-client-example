...
export function __tmp_wrap_presenceStore(context) {
    ...
    // export
    const reportLocations = (locations) => sendMessage({ type: 'state', locations: locations });
    const requestRollCall = () => sendMessage({ type: 'rollCall' });
    const rollCallRequests$ = presenceEvents$.pipe(filter((event) => event.type === 'rollCall'), 
    ...
    const reportLocation$ = defer(() => merge(locationChange$, rollCallRequests$)).pipe(switchMap(() => reportLocationInterval$), withLatestFrom(currentLocation$), map(([, locations]) => locations), auditTime(200), switchMap((locations) => reportLocations(locations)), mergeMapTo(EMPTY), share());
    ...
    // export
    const debugPresenceParam$ = debugParams$.pipe(map((args) => args.find((arg) => arg.startsWith('presence='))), map((arg) => arg
        ?.split('presence=')[1]
        .split(',')
        .map((r) => r.trim()) || []));
    ...
    const states$ = merge(syncEvent$, useMock$).pipe(scan((keyed, event) => event.type === 'disconnect'
    ...
    const allSessions$ = connectionChange$.pipe(switchMap((status) => (status === 'connected' ? merge(states$, reportLocation$) : NEVER)), map((keyedSessions) => Object.values(keyedSessions)), switchMap((sessions) => {
        ...
        return from(userStore.getUsers(userIds)).pipe(map((users) => sessions
            .map((session) => ({
    ...
    const globalPresence$ = allSessions$.pipe(map((sessions) => {
        const grouped = groupBy(sessions.map((s) => s.session), (e) => e.userId);
        return Object.keys(grouped).map((userId) => ({
            user: sessions.find((s) => s.user.id === userId)?.user,
            sessions: grouped[userId],
        }));
    }), withLatestFrom(debugIntrospect$), map(([userAndSessions, debugIntrospect]) => userAndSessions.filter((userAndSession) => {
        if (debugIntrospect) {
            return true;
        }
    ...
    const documentPresence = (documentId) => {
        return allSessions$.pipe(withLatestFrom(debugIntrospect$), switchMap(([userAndSessions, debugIntrospect]) => from(userAndSessions).pipe(filter((userAndSession) => debugIntrospect || userAndSession.session.sessionId !== SESSION_ID), flatMap((userAndSession) => (userAndSession.session.locations || [])
