 ...
 * @internal
 */
export function validateNames(workspaces) {
    ...
    workspaces.forEach((workspace, index) => {
        ...
        if (!rawName && !isSingleWorkspace) {
            throw new WorkspaceValidationError('All workspaces must have a `name`, unless only a single workspace is defined. ' +
                `Workspace ${thisIdentifier} did not define a \`name\`.`, { workspace, index });
        }
        const name = isSingleWorkspace && typeof rawName === 'undefined' ? 'default' : rawName;
        if (typeof name !== 'string') {
            throw new WorkspaceValidationError(`Workspace at index ${index} defined an invalid \`name\` - must be a string.`, { workspace, index });
        }
        ...
        if (existingWorkspace) {
            ...
            throw new WorkspaceValidationError(`\`name\`s must be unique. Workspace ${prevIdentifier} and ` +
                `workspace ${thisIdentifier} both have the \`name\` \`${name}\``, { workspace, index });
        }
        names.set(normalized, { index, workspace });
        if (!/^[a-z0-9][a-z0-9_-]*$/i.test(name)) {
            throw new WorkspaceValidationError(`All workspace \`name\`s must consist of only a-z, 0-9, underscore and dashes, ` +
                `and cannot begin with an underscore or dash. ` +
                `Workspace ${thisIdentifier} has the invalid name \`${name}\``, { workspace, index });
        }
    });
}
/**
 ...
 * @internal
 */
export function validateBasePaths(workspaces) {
    ...
    restOfWorkspaces.forEach((workspace, index) => {
        ...
        if (firstWorkspaceSegmentCount !== workspaceSegmentCount) {
            throw new WorkspaceValidationError(`All workspace \`basePath\`s must have the same amount of segments. Workspace \`${getIdentifier(firstWorkspace, index)}\` had ${firstWorkspaceSegmentCount} segment${firstWorkspaceSegmentCount === 1 ? '' : 's'} \`${firstWorkspace.basePath}\` but workspace \`${getIdentifier(workspace, index)}\` had ${workspaceSegmentCount} segment${workspaceSegmentCount === 1 ? '' : 's'} \`${workspace.basePath}\``, { workspace, index });
        }
    });
    ...
    workspaces.forEach((workspace, index) => {
        ...
        if (existingWorkspace) {
            throw new WorkspaceValidationError(`\`basePath\`s must be unique. Workspaces \`${existingWorkspace}\` and ` +
                `\`${getIdentifier(workspace, index)}\` both have the \`basePath\` \`${basePath}\``, { workspace, index });
        }
...
function hasBasePath(workspace, index) {
    ...
    if (typeof basePath === 'undefined') {
        throw new WorkspaceValidationError(`If more than one workspace is defined, every workspace must have a \`basePath\` defined. ` +
            `Workspace \`${name}\` is missing a \`basePath\``, { workspace, index });
    }
    throw new WorkspaceValidationError(`If more than one workspace is defined, every workspace must have a \`basePath\` defined. ` +
        `Workspace \`${name}\` has an invalid \`basePath\` (must be a non-empty string)`, { workspace, index });
}
function validateBasePath(workspace, index) {
    ...
    if (!/^\/[a-z0-9/_-]*[a-z0-9_-]+$/i.test(basePath)) {
        throw new WorkspaceValidationError(`All workspace \`basePath\`s must start with a leading \`/\`, ` +
            `consist of only URL safe characters, ` +
            `and cannot end with a trailing \`/\`. ` +
            `Workspace \`${name}\`'s basePath is \`${basePath}\``, { workspace, index });
    }
}
/**
 ...
 * @internal
 */
function getIdentifier({ name, title }, index) {
...
//# sourceMappingURL=validateWorkspaces.js.map