...
export function validateTemplates(schema, templates) {
    const idMap = new Map();
    templates.forEach((template, i) => {
        ...
        if (typeof template.value !== 'function' && !isRecord(template.value)) {
            throw new Error(`Template ${id} has an invalid "value" property; should be a function or an object`);
        }
        ...
        if (idMap.has(template.id)) {
            ...
            throw new Error(`Template "${template.title}" at index ${i} has the same ID ("${template.id}") as template ${dupe}`);
        }
        idMap.set(template.id, i);
    });
    return templates;
}
export function validateInitialObjectValue(value, template) {
    ...
    if (value._type && template.schemaType !== value._type) {
        throw new Error(contextError(`includes "_type"-property (${value._type}) that does not match template (${template.schemaType})`));
    }
    try {
    ...
    if (Array.isArray(value)) {
        return value.map((item, i) => {
            if (Array.isArray(item)) {
                throw new Error(`multidimensional arrays are not supported (at path "${pathToString(path)}")`);
            }
...
function validateParameter(schema, parameter, template, index) {
    ...
    if (!parameter.name) {
        throw new Error(`Template ${template.id} has a parameter at index ${index} that is missing its "name"-property`);
    }
    // I know, this is a weird one
    if (parameter.name === 'template') {
        throw new Error(`Template parameters cannot be named "template", see parameter #${index} for template ${template.id}`);
    }
    if (!schema.get(parameter.type)) {
        throw new Error(`Template parameter "${parameter.name}" has an invalid/unknown type: "${parameter.type}"`);
    }
}
function validateReference(value, path = []) {
    if (!value._type && value.type) {
        throw new Error(`Reference is missing "_type", but has a "type" property at path "${pathToString(path)}"`);
    }
    ...
    if (disallowed.length > 0) {
        ...
        throw new Error(`Disallowed ${plural} found in reference: ${disallowed
            .map(quote)
