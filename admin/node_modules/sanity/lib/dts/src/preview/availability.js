...
import { AVAILABILITY_NOT_FOUND, AVAILABILITY_PERMISSION_DENIED, AVAILABILITY_READABLE, } from './constants';
...
export function create_preview_availability(versionedClient, observePaths) {
    /**
    ...
    function observeDocumentPairAvailability(value) {
        ...
        ]).pipe(distinctUntilChanged(shallowEquals), map(([draftReadability, publishedReadability]) => {
            return {
    ...
    function observeDocumentAvailability(value) {
        ...
        // check for existence
        return observePaths(value, [['_rev']]).pipe(map((res) => isRecord(res) && Boolean('_rev' in res && res?._rev)), distinctUntilChanged(), switchMap((hasRev) => {
            return hasRev
    ...
    const fetchDocumentReadability = debounceCollect(function fetchDocumentReadability(args) {
        const uniqueIds = [...new Set(flatten(args))];
        return from(chunkDocumentIds(uniqueIds)).pipe(mergeMap(fetchDocumentReadabilityChunked, 10), map((res) => args.map(([id]) => res[uniqueIds.indexOf(id)])));
    }, 1);
    function fetchDocumentReadabilityChunked(ids) {
        return defer(() => {
            ...
            return versionedClient.observable.request(requestOptions).pipe(map((response) => {
