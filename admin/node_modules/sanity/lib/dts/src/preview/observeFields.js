...
export function create_preview_observeFields(context) {
    ...
    const getGlobalEvents = () => {
        if (!_globalListener) {
            ...
            const welcome$ = allEvents$.pipe(filter((event) => event.type === 'welcome'), publishReplay(1), refCount());
    ...
    const getBatchFetcherForDataset = memoize(function getBatchFetcherForDataset(apiConfig, token) {
    ...
    function crossDatasetListenFields(id, fields, apiConfig) {
        ...
        }, ['token']).pipe(map((document) => document?.token));
        return combineLatest([token$, visiblePoll$.pipe(startWith(0))]).pipe(switchMap(([token]) => {
    ...
    function createCachedFieldObserver(id, fields, apiConfig) {
        let latest = null;
        const changes$ = merge(defer(() => (latest === null ? EMPTY : observableOf(latest))), (apiConfig
            ? crossDatasetListenFields(id, fields, apiConfig)
            : currentDatasetListenFields(id, fields))).pipe(tap((v) => (latest = v)), publishReplay(1), refCount());
    ...
    function cachedObserveFields(id, fields, apiConfig) {
        ...
        const missingFields = difference(fields, flatten(existingObservers.map((cachedFieldObserver) => cachedFieldObserver.fields)));
...
//# sourceMappingURL=observeFields.js.map