...
export function useVisibility(props) {
    ...
    useEffect(() => {
        ...
        const isDocumentVisible$ = concat(of(!document.hidden), visibilityChange$.pipe(map((event) => (event.target instanceof Document ? !event?.target?.hidden : false)))).pipe(distinctUntilChanged());
