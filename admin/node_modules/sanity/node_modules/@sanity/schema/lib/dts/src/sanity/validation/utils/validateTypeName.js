...
export function validateTypeName(typeName, visitorContext) {
    ...
    if (!typeName) {
        return [
            error(`Type is missing a type. Valid types are: ${humanize(possibleTypeNames)}`, HELP_IDS.TYPE_MISSING_TYPE),
        ];
    }
    if (typeof typeName !== 'string') {
        return [
            error(`Type has an invalid "type"-property - should be a string. Valid types are: ${humanize(possibleTypeNames)}`, HELP_IDS.TYPE_MISSING_TYPE),
        ];
    }
