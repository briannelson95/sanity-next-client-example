...
const readActions = (schemaType) => {
    if (DEPRECATE_EXPERIMENTAL_ACTIONS && !(schemaType.name in hasWarned)) {
        console.warn(`Heads up! Experimental actions is now deprecated and replaced by Document Actions. Read more about how to migrate on ${generateHelpUrl('experimental-actions-replaced-by-document-actions')}".
`);
...
export const resolveEnabledActions = (schemaType) => validateActions(schemaType.name, readActions(schemaType));
export const isActionEnabled = (schemaType, action) => resolveEnabledActions(schemaType).includes(action);
