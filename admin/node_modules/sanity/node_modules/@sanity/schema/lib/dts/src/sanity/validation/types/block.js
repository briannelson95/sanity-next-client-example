...
export default function validateBlockType(typeDef, visitorContext) {
    ...
    if (disallowedKeys.length > 0) {
        problems.push(error(`Found unknown properties for block declaration: ${humanizeList(disallowedKeys.map(quote))}`));
    }
    if (marks) {
...
function validateMarks(marks, visitorContext, problems) {
    ...
    if (disallowedMarkKeys.length > 0) {
        problems.push(error(`Found unknown properties for block declaration: ${humanizeList(disallowedMarkKeys.map(quote))}`));
    }
...
function validateStyles(styles, visitorContext, problems) {
    ...
    styles.forEach((style, index) => {
        ...
        if (disallowedKeys.length > 0) {
            problems.push(error(`Found unknown properties for style ${name}: ${humanizeList(disallowedKeys.map(quote))}`));
        }
        if (!style.value) {
...
function validateDecorators(decorators, visitorContext, problems) {
    decorators.forEach((decorator, index) => {
        ...
        if (disallowedKeys.length > 0) {
            problems.push(error(`Found unknown properties for decorator ${name}: ${humanizeList(disallowedKeys.map(quote))}`));
        }
        if (!decorator.value) {
