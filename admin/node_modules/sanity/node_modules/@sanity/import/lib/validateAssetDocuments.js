...
module.exports = async function validateAssetDocuments(docs, options) {
  ...
  if (!options.allowAssetsInDifferentDataset) {
    assetDocs.forEach(doc => {
      ...
      const resolveText = "See ".concat(generateHelpUrl('import-asset-has-different-target'));

      if (projectId !== targetProjectId) {
        throw new Error("Asset ".concat(id, " references a different project ID than the specified target (asset is in ").concat(projectId, ", importing to ").concat(targetProjectId, "). ").concat(resolveText));
      }

      if (dataset !== targetDataset) {
        throw new Error("Asset ".concat(id, " references a different dataset than the specified target (asset is in ").concat(dataset, ", importing to ").concat(targetDataset, "). ").concat(resolveText));
      }
    });
  }
...
async function ensureAssetUrlExists(assetDoc) {
  ...
  debug("".concat(url, ": %s (%d ms)"), exists ? 'exists' : 'does not exist', Date.now() - start);

  if (!exists) {
    const helpUrl = generateHelpUrl('import-asset-file-does-not-exist');
    throw new Error("Document ".concat(assetDoc._id, " points to a URL that does not exist (").concat(url, "). See ").concat(helpUrl, "."));
  }
...
function validateAssetDocumentProperties(assetDoc) {
  Object.keys(REQUIRED_PROPERTIES).forEach(prop => {
    ...
    if (typeof assetDoc[prop] !== expectedType) {
      ...
      throw new Error("Asset document ".concat(assetDoc._id, " ").concat(errorType, " required property \"").concat(prop, "\""));
    }
  });
...
function validateImageMetadata(assetDoc) {
  if (!assetDoc.metadata) {
    throw new Error("Asset document ".concat(assetDoc._id, " is missing required property \"metadata\""));
  }

  if (!assetDoc.metadata.dimensions) {
    throw new Error("Asset document ".concat(assetDoc._id, " is missing required property \"metadata.dimensions\""));
  }
  ...
  dimensionProps.forEach(prop => {
    if (typeof assetDoc.metadata.dimensions[prop] !== 'number') {
      throw new Error("Asset document ".concat(assetDoc._id, " is missing required property \"metadata.dimensions.").concat(prop, "\""));
    }
  });
}