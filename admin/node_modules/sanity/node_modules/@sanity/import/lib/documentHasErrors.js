...
function documentHasErrors(doc) {
  ...
  if (typeof doc._id !== 'undefined' && !/^[a-z0-9_.-]+$/i.test(doc._id)) {
    return "Document ID \"".concat(doc._id, "\" is not valid: Please use alphanumeric document IDs. Dashes (-) and underscores (_) are also allowed.");
  }
...
documentHasErrors.validate = (doc, index) => {
  ...
  if (err) {
    throw new Error("Failed to parse document at index #".concat(index, ": ").concat(err));
  }
};
