...
async function uploadAssets(assets, options) {
  ...
  const ensureMethod = options.allowFailingAssets ? function () {
    return ensureAssetExists(...arguments).catch(err => err);
  } : ensureAssetExists; // Loop over all unique URLs and ensure they exist, and if not, upload them
...
function getAssetRefMap(assets) {
  return assets.reduce((assetRefMap, item) => {
    ...
    const key = "".concat(type, "#").concat(url);
...
async function ensureAssetWithRetries(options, progress, assetKey, i) {
  ...
  const {
    ...
    err.type = type;
    err.url = url;
    err.message = err.message.includes(url) ? err.message : "Failed to download ".concat(type, " @ ").concat(url, ":\n").concat(err.message);
    throw err;
  });
  const asset = {
    buffer,
  ...
  return retryOnFailure(() => ensureAsset(asset, options, i)).then(progress).catch(err => {
    ...
    err.type = type;
    err.url = url;
    err.message = err.message.includes(url) ? err.message : "Failed to upload ".concat(type, " @ ").concat(url, ":\n").concat(err.message);
    throw err;
  });
}
...
async function ensureAsset(asset, options, i) {
  ...
  const assetMeta = assetMap["".concat(type, "-").concat(sha1hash)];
...
async function getAssetDocumentIdForHash(client, type, sha1hash, attemptNum, tag) {
  ...
  try {
    ...
    const assetUrl = isSanityImageUrl(assetDoc.url) ? "".concat(assetDoc.url, "?fm=json") : assetDoc.url;
    const exists = await urlExists(assetUrl);

    if (!exists) {
      debug("Asset document ".concat(assetDoc._id, " exists, but file does not. Overwriting."));
      return null;
    }
...
function setAssetReferences(assetRefMap, assetIds, options) {
  ...
  const patchTasks = assetIds.reduce((tasks, assetId) => {
    ...
    return tasks.concat(documents.map(_ref2 => {
      let {
        documentId,
        path
      } = _ref2;
