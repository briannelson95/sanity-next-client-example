...
function absolutifyPaths(doc, absPath) {
  ...
  const modifier = value => value.replace(/file:\/\/\.\//i, "".concat(getFileUrl(absPath, {
    resolve: false
  }), "/")).replace(/(https?):\/\/\.\//, "$1://".concat(absPath, "/"));
...
function findAssetRefs(doc) {
  return extractWithPath("..[".concat(assetKey, "]"), doc).map(match => match.path);
}

function validateAssetImportKey(path, doc) {
  if (!assetMatcher.test(get(doc, path))) {
    throw new Error(['Asset type is not specified.', '`_sanityAsset` values must be prefixed with a type, eg image@url or file@url.', "See document with ID \"".concat(doc._id, "\", path: ").concat(serializePath({
      path
    }))].join('\n'));
  }
