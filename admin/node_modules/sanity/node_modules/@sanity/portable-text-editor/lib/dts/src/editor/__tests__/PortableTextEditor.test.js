...
describe('initialization', () => {
    it('receives initial onChange events and has custom placeholder', () => {
        ...
        const { container } = render(React.createElement(PortableTextEditorTester, { onChange: onChange, renderPlaceholder: () => 'Jot something down here', ref: editorRef, type: type, value: undefined }));
    ...
    it('updates editor selection from new prop and keeps object equality in editor.getSelection()', () => {
        ...
        const onChange = jest.fn();
        const { rerender } = render(React.createElement(PortableTextEditorTester, { onChange: onChange, ref: editorRef, selection: initialSelection, type: type, value: initialValue }));
        if (!editorRef.current) {
            throw new Error('No editor');
        }
        act(() => {
