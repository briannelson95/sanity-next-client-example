...
describe('schema validation inference', () => {
    ...
    describe('slug validation', () => {
        ...
        test('slug is valid if uniqueness queries returns true', async () => {
            ...
            true));
            await expect(validateDocument(client, mockDocument, schema)).resolves.toEqual([]);
        ...
        test('slug is invalid if uniqueness queries returns false', async () => {
            ...
            false));
            await expect(validateDocument(client, mockDocument, schema)).resolves.toMatchObject([
                {
                    path: ['slugField'],
                    level: 'error',
    ...
    describe('reference validation', () => {
        ...
        test('referenced document must exist (unless weak)', async () => {
            ...
            await expect(validateDocument(client, {
                ...mockDocument,
                referenceField: {
                    _ref: 'example-id',
                },
        ...
        test('throws if `getDocumentExists` is not present', async () => {
            const result = await validateDocument(client, {
                ...mockDocument,
                referenceField: {
                    _ref: 'example-id',
                },
...
async function expectNoError(validations, value) {
    const errors = (await Promise.all(validations.map((rule) => rule.validate(value, { client: {} })))).flat();
    if (errors.length === 0) {
