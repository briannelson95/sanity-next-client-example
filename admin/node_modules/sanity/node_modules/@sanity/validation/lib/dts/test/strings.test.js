...
describe('string', () => {
    ...
    test('exact length constraint', async () => {
        ...
        await expect(rule.validate('abcdefgh', context)).resolves.toMatchSnapshot('exact length: too long');
    ...
    test('uppercase constraint', async () => {
        ...
        await expect(rule.validate('sanity', context)).resolves.toMatchSnapshot('uppercase: all lowercase');
        await expect(rule.validate('Sanity', context)).resolves.toMatchSnapshot('uppercase: some lowercase');
        await expect(rule.validate('Sanity', context)).resolves.toMatchSnapshot('uppercase: some lowercase');
        await expect(rule.validate('SäNITY', context)).resolves.toMatchSnapshot('uppercase: locale characters');
    ...
    test('lowercase constraint', async () => {
        ...
        await expect(rule.validate('SANITY', context)).resolves.toMatchSnapshot('lowercase: all uppercase');
        await expect(rule.validate('Sanity', context)).resolves.toMatchSnapshot('lowercase: some uppercase');
        await expect(rule.validate('sÄnity', context)).resolves.toMatchSnapshot('lowercase: locale characters');
    ...
    test('regex constraint (inverted)', async () => {
        ...
        await expect(rule.validate('SANITY', context)).resolves.toMatchSnapshot('regex: inverted non-match');
    ...
    test('regex constraint (custom pattern name)', async () => {
        ...
        await expect(rule.validate('SANITY', context)).resolves.toMatchSnapshot('regex: non-match w/ custom pattern name');
        await expect(rule.validate('Sanity', context)).resolves.toMatchSnapshot('regex: match w/ custom pattern name');
    });
    test('regex constraint (custom pattern name, as options)', async () => {
        const rule = Rule.string().regex(/^[A-Z][a-z]+$/, { name: 'PascalCase' });
        await expect(rule.validate('SANITY', context)).resolves.toMatchSnapshot('regex: non-match w/ custom pattern name (opt)');
        await expect(rule.validate('Sanity', context)).resolves.toMatchSnapshot('regex: match w/ custom pattern name (opt)');
    });
    ...
    test('uri constraint (with unicode chars)', async () => {
        const rule = Rule.string().uri();
        await expect(rule.validate('Blåbærsyltetøy', context)).resolves.toMatchSnapshot('uri: non-match');
        await expect(rule.validate('https://en.wikipedia.org/wiki/San_&_Søn', context)).resolves.toHaveLength(0);
        await expect(rule.validate('https://zh.wikipedia.org/wiki/心形符號', context)).resolves.toHaveLength(0);
        await expect(rule.validate('https://ru.wikipedia.org/wiki/Зонтичные', context)).resolves.toHaveLength(0);
    ...
    test('uri constraint (invalid protocol)', async () => {
        ...
        await expect(rule.validate('https://sanity.io/', context)).resolves.toMatchSnapshot('uri: protocol non-match');
    ...
    test('uri constraint (credentials)', async () => {
        ...
        await expect(rule.validate('http://sanity.io/', context)).resolves.toHaveLength(0);
        await expect(rule.validate('http://foo:bar@sanity.io/', context)).resolves.toMatchSnapshot('uri: credentials specified but not allowed');
        await expect(rule.validate('http://espen@sanity.io/', context)).resolves.toMatchSnapshot('uri: username specified but not allowed');
    });
    test('custom rule with string', async () => {
        const rule = Rule.string().custom((val) => val.split('').reverse().join('') === val ? true : 'Must be a palindrome!');
    ...
    test('custom async rule with string', async () => {
        const rule = Rule.string().custom((val) => new Promise((resolve) => setTimeout(() => resolve(val.split('').reverse().join('') === val ? true : 'Must be a palindrome!'), 50)));
        await expect(rule.validate('hei', context)).resolves.toMatchSnapshot('not a palindrome');
        await expect(rule.validate('madam', context)).resolves.toHaveLength(0);
    });
});
