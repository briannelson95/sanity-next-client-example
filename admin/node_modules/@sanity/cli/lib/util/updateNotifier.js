...
function runUpdateCheck(options) {
  (0, _debug.debug)('CLI installed at %s', __dirname);
  const {
    pkg,
    cwd,
    workDir
  ...
  async function notify() {
    ...
    if (hasPrintedResult) {
      (0, _debug.debug)('Has already printed result through timeout check, skipping result notification');
      return;
    }
  ...
  async function printCachedResult() {
    (0, _debug.debug)('Max time (%dms) reached waiting for latest version info', MAX_BLOCKING_TIME);
    hasPrintedResult = true;
    const cached = userConfig.get('cliHasUpdate');

    if (!cached) {
      (0, _debug.debug)('No cached latest version result found');
      return false;
    }
    ...
    if (diff <= 0) {
      ...
      (0, _debug.debug)('CLI was upgraded since last check, falling back');
      userConfig.delete('cliHasUpdate');
      return false;
    }

    (0, _debug.debug)('Printing cached latest version result');
    await printResult(cached);
    return false;
  }

  async function printResult(newVersion) {
    hasPrintedResult = true;
    const lastUpdated = userConfig.get('cliLastUpdateNag') || 0;

    if (Date.now() - lastUpdated < TWELVE_HOURS) {
      (0, _debug.debug)('Less than 12 hours since last nag, skipping');
      return;
    }

    if (!newVersion || (0, _semverCompare.default)(newVersion, version) <= 0) {
      (0, _debug.debug)("New version is ".concat(newVersion || 'unknown', ", current is ").concat(version, ". Falling back."));
      return;
    }

    const upgradeCommand = await (0, _packageManager.getCliUpgradeCommand)({
      cwd,
      workDir
    });
    const message = ['Update available ', _chalk.default.dim(version), _chalk.default.reset(' → '), _chalk.default.green(newVersion), ' \nRun ', _chalk.default.cyan(upgradeCommand), ' to update'].join('');
    const boxenOpts = {
      padding: 1,
      margin: 1,
      borderColor: 'yellow',
    ...
    console.error("\n".concat((0, _boxen.default)(message, boxenOpts)));
    userConfig.set('cliLastUpdateNag', Date.now());
  }

  async function getLatestRemote() {
    if (isDisabled) {
      (0, _debug.debug)('Running on CI, or explicitly disabled, skipping update check');
      return false;
    }
    ...
    if (Date.now() - lastUpdated < TWELVE_HOURS) {
      (0, _debug.debug)('Less than 12 hours since last check, skipping update check');
    ...
    try {
      (0, _debug.debug)('Checking for latest remote version');
      latestRemote = await (0, _getLatestVersion.default)(name);
      (0, _debug.debug)('Latest remote version is %s', latestRemote);
    } catch (err) {
      (0, _debug.debug)("Failed to fetch latest version of ".concat(name, " from npm:\n").concat(err.stack));
      return false;
    }
    ...
    if (diff <= 0) {
      // No change, or lower
      (0, _debug.debug)(diff === 0 ? 'No update found' : 'Remote version older than local');
      userConfig.delete('cliHasUpdate');
      return false;
    ...
    (0, _debug.debug)('Update is available (%s)', latestRemote);
    return latestRemote;
  }
}