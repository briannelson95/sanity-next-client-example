...
export async function runCli(cliRoot, { cliVersion }) {
...
async function getCoreModulePath(workDir, cliConfig) {
    ...
    if (corePath && sanityPath) {
        ...
        console.warn(chalk.yellow(`Both \`@sanity/core\` AND \`sanity\` installed - assuming Sanity ${assumedVersion} project.`));
        return closest;
    }
    if (sanityPath) {
...
function warnOnNonProductionEnvironment() {
    ...
    console.warn(chalk.yellow(knownEnvs.includes(sanityEnv)
