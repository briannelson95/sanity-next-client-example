...
export default async function initSanity(args, context) {
    ...
    const sendInvite = isFirstProject &&
        (await prompt.single({
            type: 'confirm',
    ...
    async function getOrCreateUser() {
        ...
        const loginArgs = { ...otherArgs, extOptions: {} };
    ...
    async function getOrCreateProject() {
        ...
        if (flags.organization) {
            const organization = organizations.find((org) => org.id === flags.organization) ||
    ...
    async function getOrCreateDataset(opts) {
        ...
        if (opts.dataset) {
            ...
            if (!existing) {
                ...
                await client.datasets.create(opts.dataset, { aclMode: aclMode });
                spinner.succeed();
            }
            return { datasetName: opts.dataset };
        }
        ...
        if (datasets.length === 0) {
            ...
            const aclMode = await getAclMode();
            const spinner = context.output.spinner('Creating dataset').start();
            await client.datasets.create(name, { aclMode: aclMode });
            spinner.succeed();
        ...
        if (selected === 'new') {
            ...
            const aclMode = await getAclMode();
            const spinner = context.output.spinner('Creating dataset').start();
            await client.datasets.create(newDatasetName, { aclMode: aclMode });
            spinner.succeed();
...
function getImportCommand(outputPath, studioVersion) {
    if (studioVersion === 3) {
        ...
        const cliInternals = dynamicRequire(pkgPath);
        if (!('cliProjectCommands' in cliInternals)) {
            throw new Error('Incorrect version of the `sanity` module installed');
        }
        return cliInternals.cliProjectCommands.commands.find((cmd) => !isCommandGroup(cmd) && cmd.name === 'import' && cmd.group === 'dataset');
    }
    ...
    const coreModule = dynamicRequire(pkgPath);
    ...
    return coreCommands.find((cmd) => !isCommandGroup(cmd) && cmd.name === 'import' && cmd.group === 'dataset');
}
