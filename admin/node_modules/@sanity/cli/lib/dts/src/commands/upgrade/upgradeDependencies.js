...
const upgradeDependencies = async function upgradeDependencies(args, context) {
    ...
    await Promise.all(nonPinned.map((mod) => deleteIfNotSymlink(path.join(context.workDir, 'node_modules', mod.name.replace(/\//g, path.sep)))));
...
function schedulePrintMajorUpgrades({ baseMajorUpgrade, majorUpgrades, }, { chalk, output }) {
    ...
    process.on('beforeExit', () => {
        ...
        if (baseMajorUpgrade) {
            output.warn(boxen(getMajorStudioUpgradeText(baseMajorUpgrade, chalk), {
                borderColor: 'green',
                padding: 1,
            }));
            return;
        }
        output.warn(boxen(getMajorUpgradeText(majorUpgrades, chalk), {
            borderColor: 'yellow',
            padding: 1,
        }));
    });
...
async function maybeDeleteReactAce(toUpgrade, workDir) {
    ...
    const closestReactAcePath = getModulePath('react-ace', path.join(depRootPath, '@sanity', 'code-input')) ||
